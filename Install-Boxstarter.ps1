function Get-Boxstarter {
    ...
    if(Check-Chocolatey -Force:$Force){
        ...
        try {
            ...
            $command = "cinst Boxstarter.Common -y"
        }
        catch{
            # if there is no -v then its an older version with no -y
            $command = "cinst Boxstarter.Common"
        }
        ...
        try {
            New-Object -TypeName Version -ArgumentList $version.split('-')[0] | Out-Null
            $command = "cinst Boxstarter.WinConfig -y"
        }
        catch{
            # if there is no -v then its an older version with no -y
            $command = "cinst Boxstarter.WinConfig"
        }
        $command += " -version 2.8.29"
        Invoke-Expression $command
        ...
        try {
            New-Object -TypeName Version -ArgumentList $version.split('-')[0] | Out-Null
            $command = "cinst Boxstarter.Bootstrapper -y"
        }
        catch{
            # if there is no -v then its an older version with no -y
            $command = "cinst Boxstarter.Bootstrapper"
        }
        $command += " -version 2.8.29"
        Invoke-Expression $command
        ...
        try {
            New-Object -TypeName Version -ArgumentList $version.split('-')[0] | Out-Null
            $command = "cinst Boxstarter.Chocolatey -y"
        }
        catch{
            # if there is no -v then its an older version with no -y
            $command = "cinst Boxstarter.Chocolatey"
        }
        $command += " -version 2.8.29"
        Invoke-Expression $command
        ...
        try {
            New-Object -TypeName Version -ArgumentList $version.split('-')[0] | Out-Null
            $command = "cinst Boxstarter.HyperV -y"
        }
        catch{
            # if there is no -v then its an older version with no -y
            $command = "cinst Boxstarter.HyperV"
        }
        $command += " -version 2.8.29"
        Invoke-Expression $command
